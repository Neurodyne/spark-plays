...
object KafkaWordCountDelay {
  def main(args: Array[String]) {
    ...
    val lines =
      KafkaUtils.createStream(ssc, zkQuorum, group, topicMap /*, StorageLevel.MEMORY_AND_DISK_SER*/ ).map(_._2)
    ...
    val wordCounts =
      words.map(x => (x, 1L)).reduceByKeyAndWindow(_ + _, _ - _, Seconds(10), Seconds(2), 2).filter(x => x._2 > 0)
...
object KafkaWordCountProducerBatch {
  def main(args: Array[String]) {
    if (args.length < 5) {
      System.err.println(
        "Usage: <metadataBrokerList> <topic> " +
