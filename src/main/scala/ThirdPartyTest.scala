...
//     val movieToRatings =
//       movieToUserRating.aggregateByKey(0.0)((sum, ur) => sum + pow(ur._2, 2.0), _ + _).map(x => (x._1, sqrt(x._2)))

//     val userToMovieRatingReg = movieToUserRating
//       .join(movieToRatings)
//       .map({
...
//     val movieToMovieScore = userToMovieRatingReg
//       .join(userToMovieRatingReg)
//       .map({
