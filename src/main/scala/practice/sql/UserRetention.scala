...
object UserRetention {
  ...
  private def init(statDay: String, hiveSqlContext: HiveContext): Unit = {
    ...
    if (Params.hiveStatdayDelete) {
      ...
      fileSystem.delete(
        new Path(s"${Params.hiveWarehouseDir}/${Params.hiveAlluserCreateTable}/${DATE}=${statDay}"),
        true
      )
      fileSystem.delete(
        new Path(s"${Params.hiveWarehouseDir}/${Params.hiveNewuserCreateTable}/${DATE}=${statDay}"),
        true
      )
    }
    ...
    // 做一个索引
    MongoDBConnection.getInstance.collConn
      .createIndex(MongoDBObject(APPKEY -> true, BASE_DAY -> true, STAT_DAY -> true))
  }
   ...
   * @param allUserDF
   */
  private def computeCurrentAllAndNewUser(
    hiveSqlContext: HiveContext,
    statDay: String,
    rawDF: DataFrame,
    allUserDF: DataFrame
  ): Unit = {
  ...
  private def saveMongodb(r: DataFrame, baseDay: String, statDay: String): Unit =
    r.foreachPartition { partitionOfRecords =>
  ...
  def main(args: Array[String]): Unit = {
    ...
    val rawDF =
      rawDF0.repartition(Params.sourcedataPartition, rawDF0(APPKEY), rawDF0(GID)).persist(StorageLevel.MEMORY_AND_DISK)
