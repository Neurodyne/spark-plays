...
object TextCategoryV16 {
  ...
  private def constructPipeline(sqlContext: SQLContext, data: Option[DataFrame] = None): Pipeline = {
    ...
    // 对标题进行中文分词
    val tokenizer = new ChineseSegment(Option(Params.userDict)).setInputCol(TITLE).setOutputCol(TITLE_WORDS)

    // 处理单位词信息, 之后对数词也做一下过滤
    val quantifier =
      new QuantifierProcess(Params.preprocessQuantifierFile).setInputCol(TITLE_WORDS).setOutputCol(QUAN_TITLE_WORDS)
    ...
    val sqlTrans = new SQLTransformer()
      .setStatement(s"SELECT *, strArrayMerge(${BRAND}, ${MED_QUAN_TITLE_WORDS}) AS ${BRAND_WORDS} FROM __THIS__")
    ...
    if (data != None) {
      ...
      val pl = Params.commonVecSpace match {
        case c: String if (c == "word" || c == "topic") =>
          new Pipeline().setStages(Array(tokenizer, quantifier, medicine, sqlTrans, remover, tf))
        case _ =>
          new Pipeline().setStages(Array(tokenizer, quantifier, medicine, sqlTrans, remover))
      }
