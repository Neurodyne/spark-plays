...
object GraphOperation {
  def printGraph[VD, ED](graph: Graph[VD, ED], hint: String): Unit = {
    ...
    graph.triplets
      .map(triplet =>
        s"<${triplet.srcId},${triplet.srcAttr}> is the ${triplet.attr} of <${triplet.dstId},${triplet.dstAttr}>"
      )
      .collect
      .foreach(println)
  }
  ...
  def main(args: Array[String]): Unit = {
    ...
    // 定义 vertex 的 RDD
    val users: RDD[(VertexId, Person)] = ctx.parallelize(
      Array(
    ...
    // 定义 edge 的 RDD
    val relationships: RDD[Edge[String]] = ctx.parallelize(
      Array(
        Edge(1L, 2L, "advisor"),
        Edge(1L, 3L, "advisor"),
        Edge(2L, 3L, "friend"),
    ...
    val graph2 = graph.mapVertices { case (id, vd) => if (vd.name == "qi.liu") Person(vd.name, "prof", vd.age) else vd }
    ...
    val u2: RDD[(VertexId, String)] = ctx.parallelize(
      Array(
        (2L, "prof"),
        (3L, "staff")
      )
    ...
    graph.collectNeighbors(EdgeDirection.Out).collect.foreach {
      case (id, neighboors) => {
        val s = for (nei <- neighboors) yield nei._1
        println(s"$id -- ${s.mkString(", ")}")
      }
    ...
    graph.collectNeighbors(EdgeDirection.Either).collect.foreach {
      case (id, neighboors) => {
        val s = for (nei <- neighboors) yield nei._1
        println(s"$id -- ${s.mkString(", ")}")
      }
    ...
    val relationships2: RDD[Edge[Int]] = ctx.parallelize(
      Array(
        Edge(1L, 2L, 3),
        Edge(1L, 3L, 3),
        Edge(2L, 3L, 2),
    ...
    val sssp = initialGraph.pregel(Double.PositiveInfinity)(
      (id, dist, newDist) => linalg.min(dist, newDist), // Vertex Program
      triplet =>
        // Send Message
