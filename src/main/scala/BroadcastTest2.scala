...
object BroadcastTest2 {
  def main(args: Array[String]): Unit = {
    ...
    val sparkConf = new SparkConf()
      .setAppName("BroadcastTest2")
      .set("spark.serializer", "org.apache.spark.serializer.KryoSerializer")
      .registerKryoClasses(Array(classOf[Student]))

    val students = Map(
      "qifeng.dai" -> new Student("qifeng.dai", "SA08011084", "02-22"),
      "yijing.liu" -> new Student("yijing.liu", "BA08011001", "01-05")
    )
    ...
    val newStudents = ctx
      .parallelize(List("jessical", "matrix", "qifeng.dai"))
      .map(x =>
        broadcastVar.value.get(x) match {
          case None    => (x, None)
